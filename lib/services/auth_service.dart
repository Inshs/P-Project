...
class AuthService {
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    if (_useMock) {
      ...
      _userId = '1';
      await _saveToken();
      return {'success': true, 'message': '로그인 성공 (Mock)'};
    }
  ...
  Future<Map<String, dynamic>> sendVerificationCode(String email) async {
    if (_useMock) {
      ...
      return {'success': true, 'message': '인증 코드가 발송되었습니다 (Mock: 123456)'};
    }
  ...
  Future<Map<String, dynamic>> verifyCode(String email, String code) async {
    if (_useMock) {
      await Future.delayed(const Duration(milliseconds: 500));
      if (code == '123456') {
        return {'success': true, 'message': '인증 성공 (Mock)'};
      } else {
        return {'success': false, 'message': '인증 코드가 일치하지 않습니다 (Mock: 123456)'};
      }
    }
  ...
  Future<Map<String, dynamic>> signup(
    ...
    if (_useMock) {
      ...
      return {'success': true, 'message': '회원가입 성공 (Mock)'};
    }
  ...
  Future<Map<String, dynamic>> oauthSignup(
    ...
    if (_useMock) {
      ...
      return {
        ...
        'token': 'mock_oauth_token_${DateTime.now().millisecondsSinceEpoch}',
      };
    }
  ...
  String getSocialLoginUrl(String provider) {
    ...
    final baseUrl =
        'http://$host:8080/oauth2/authorization/${provider.toLowerCase()}';
  ...
  Future<void> handleOAuthCallback(
      String token, String email, String providerName) async {
    _token = token;
    _userEmail = email;
